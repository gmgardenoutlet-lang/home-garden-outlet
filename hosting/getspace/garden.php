...
function garden_has_display_value($value): bool
{
...
function garden_display_status(array $product): string
{
    ...
    return garden_has_display_value($product['status'] ?? '')
        ? (string)$product['status']
        : 'Dostępny od ręki';
}

function garden_is_sold(array $product): bool
{
...
function garden_images(array $product): array
{
    ...
    foreach ($paths as $path) {
        if (garden_has_display_value($path)) {
...
function garden_parse_price($value): ?float
{
...
function garden_readable_category($value): string
{
    ...
    return garden_has_display_value($value) ? (string)$value : 'Produkt outletowy';
}

function garden_image_alt(array $product, string $name): string
{
    if (garden_has_display_value($product['imageAlt'] ?? '')) {
        return trim((string)$product['imageAlt']);
    }

    $category = garden_has_display_value($product['category'] ?? '')
...
function garden_card(array $product): string
{
    $name = (string)($product['name'] ?? '') ?: 'Produkt outletowy';
    $category = garden_readable_category($product['category'] ?? 'Wyposażenie ogrodu');
    $status = garden_display_status($product);
    $images = garden_images($product);
...
$gardenProducts = $catalogIsAvailable ? catalog_products_with_slugs() : [];
...
$cards = str_replace('\\n', "\n", implode('', array_map('garden_card', $gardenProducts)));
