...
$checkoutPhoneNumber = (string) ($checkoutOld['phone_number'] ?? $checkoutOld['customer_phone'] ?? '');
    ...
    <section class="checkout-shell checkout-page" aria-label="Zamówienie">
      ...
      <form method="post" action="/sklep/order" class="checkout-form" data-checkout-form>
        ...
        <section class="checkout-step">
          ...
          <label>Telefon<div class="phone-field"<?= checkout_field_attrs($checkoutErrors, 'customer_phone') ?>><?php if (FOREIGN_SHIPPING_ENABLED): ?><select name="phone_prefix" data-phone-prefix aria-label="Kod kierunkowy telefonu" required><?php foreach (SHOP_ALLOWED_COUNTRIES as $code => $country): $prefix = '+' . $country['callingCode']; ?><option value="<?= e($prefix) ?>"<?= $checkoutPhonePrefix === $prefix ? ' selected' : '' ?>><?= e($prefix) ?></option><?php endforeach; ?></select><?php else: ?><span class="phone-prefix-static">+48</span><input type="hidden" name="phone_prefix" value="+48"><?php endif; ?><input name="phone_number" data-phone-number value="<?= e($checkoutPhoneNumber) ?>" type="tel" inputmode="tel" required maxlength="40" autocomplete="tel-national" placeholder="600 402 939"></div><?= checkout_error($checkoutErrors, 'customer_phone') ?></label>
        </section>
        <section class="checkout-step">
        ...
        <section class="checkout-step">
          ...
          <button class="btn btn-wide" type="submit" data-checkout-submit>Podsumowanie zamówienia</button>
        </section>
      </form>
    </section>
  </main>
