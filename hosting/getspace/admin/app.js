...
(() => {
  ...
  const render = () => {
    ...
    const labelEvery = rows.length <= 7 ? 1 : rows.length <= 28 ? (isNarrow ? 4 : 2) : (isNarrow ? 21 : 14);
    ...
    const drawSeries = (values, className, points) => { let segment = []; const flush = () => { if (segment.length > 1) svg.append(element("polyline", { points: segment.map((point) => point.join(",")).join(" "), class: className })); segment = []; }; values.forEach((value, index) => { if (!Number.isFinite(value)) { flush(); return; } segment.push([x(index), y(value)]); if (points) svg.append(element("circle", { cx: x(index), cy: y(value), r: 5, class: "point", fill: points })); }); flush(); };
    ...
    const helperLine = element("line", { y1: chart.top, y2: chart.height - chart.bottom, class: "hover-guide", hidden: "hidden" });
    svg.append(helperLine);
    const show = (event) => { const bounds = svg.getBoundingClientRect(); const index = Math.max(0, Math.min(rows.length - 1, Math.round(((event.clientX - bounds.left) / bounds.width) * Math.max(rows.length - 1, 0)))); const row = rows[index]; if (!row) return; const lines = ["Data: " + formatDate(row.date) + (row.date === data.today ? " · dzisiaj — dzień w toku" : ""), "Odsłony stron: " + (Number.isFinite(row.page_view) ? row.page_view : "brak danych"), "Wyświetlenia produktów: " + (Number.isFinite(row.product_view) ? row.product_view : "brak danych")]; if (averageEnabled && pageEnabled) lines.push("Średnia 7-dniowa: " + (Number.isFinite(average[index]) ? average[index].toFixed(1).replace(".", ",") : "brak danych")); tooltip.innerHTML = lines.map((line) => "<div>" + line + "</div>").join(""); tooltip.hidden = false; helperLine.setAttribute("x1", x(index)); helperLine.setAttribute("x2", x(index)); helperLine.removeAttribute("hidden"); tooltip.style.left = Math.min(root.clientWidth - 238, Math.max(8, event.clientX - bounds.left + 12)) + "px"; tooltip.style.top = "16px"; };
    const hide = () => { tooltip.hidden = true; helperLine.setAttribute("hidden", "hidden"); };
