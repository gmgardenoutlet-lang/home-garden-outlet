...
<body>
  ...
  <main class="container">
    ...
    <?php elseif ($showStats): ?>
      ...
      <section class="card stats-section"><h2>Eksport zdarzeń diagnostycznych</h2><form method="get" action="/admin/export-events.php" class="admin-filter-grid"><label>Data od<input type="date" name="from" value="<?= e(stats_today()->format('Y-m-d')) ?>" max="<?= e(stats_today()->format('Y-m-d')) ?>"></label><label>Data do<input type="date" name="to" value="<?= e(stats_today()->format('Y-m-d')) ?>" max="<?= e(stats_today()->format('Y-m-d')) ?>"></label><input type="hidden" name="type" value="<?= e($diagnosticFilters['type']) ?>"><input type="hidden" name="country" value="<?= e($diagnosticFilters['country']) ?>"><input type="hidden" name="city" value="<?= e($diagnosticFilters['city']) ?>"><input type="hidden" name="client" value="<?= e($diagnosticFilters['client']) ?>"><input type="hidden" name="device" value="<?= e($diagnosticFilters['device']) ?>"><button class="btn" type="submit">Eksportuj CSV</button></form></section>
